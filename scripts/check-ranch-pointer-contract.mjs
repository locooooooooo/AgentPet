...
assert.match(ranchAppSource, /setInteractiveRegions\(readDesktopInteractiveRegions\(dragging/);
...
assert.match(mainSource, /screen\.getCursorScreenPoint\(\)[\s\S]*cursor\.x - windowBounds\.x[\s\S]*ranchInteractiveRegions\.some/);
assert.doesNotMatch(mainSource, /x: windowBounds\.x \+ region\.x[\s\S]*y: windowBounds\.y \+ region\.y/);
...
assert.match(dockSource, /isDesktopDragHandle[\s\S]*if \(!isFloating && !\(isDesktop && isDesktopDragHandle\)\)/);
assert.match(dockSource, /target\.closest\('\[data-ranch-no-drag="true"\]'\)/);
assert.match(dockSource, /api\.ranch\.setMousePassthrough\(false\)/);
...
assert.match(cssSource, /#ranch-root \.ranch-drag-handle \{[\s\S]*?cursor: grab;/);

console.log('ranch pointer contract check passed.');
console.log('desktop=bounded hot zones + fence drag; floating=canvas drag + 32px snap + persisted dock edge');
