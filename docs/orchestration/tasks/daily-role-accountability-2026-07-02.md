...
| `[长工]#realtime-truth-ui@v0.1` | active | `docs/orchestration/tasks/realtime-agent-cockpit-p0-b-data-truth-v0.1.md` | Selector plus renderer truth wiring and browser fallback replay are PM-verified as a partial slice; renderer SSR fresh/stale/lost fixture remains incomplete and no runtime types may be duplicated. |
| `[长工]#realtime-requirements-control@v0.1` | active | `docs/orchestration/sessions/realtime-agent-cockpit-progress-2026-07-13.md` | Maintain the approximately 55% bounded baseline, A/B callback and gate matrix, and keep P0-C Codex dry-run standby pending full A/B acceptance plus a second explicit execution authorization. |
