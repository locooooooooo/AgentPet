...
| P0-B renderer | `[长工]#realtime-truth-ui@v0.1` | partial_accepted_renderer_truth_slice | App/Home/Cockpit 订阅与真值标签、SSR fresh/late/stale/session-lost/capabilities unknown fixture、1280/1440/1920 fallback 视口通过；Electron p95/真实 E2E 待补 |
...
- 真实 Electron runtime event 到 renderer 的 p95 <= 500ms 尚未测量。
...
- P0-C 必须等待 A/B full acceptance、Electron p95 验收和 PM/user 新消息中的第二次明确执行授权。
...
- 补真实 Electron runtime event p95 和 process-tree 证据；不得用 SSR fixture 或浏览器 fallback 替代真实 E2E。
- 保持 P0-C standby；没有第二次明确授权时不启用 Connector 或运行 Codex。

## callback

```text
completed: A1-A5 blocked-safe runtime、B selector、B renderer truth 接线、SSR DOM state matrix、1280/1440/1920 浏览器 fallback 复核和全门禁。
incomplete: Electron event p95、trusted authorizer、真实 reattach、P0-C/真实 Agent E2E。
blockers: machine-gate disabled；P0-C 需要 A/B full acceptance + 第二次明确执行授权。
next action: 保持 C standby，补 Electron p95/process-tree 证据，不以 SSR 或浏览器 fallback 替代真实 E2E。
evidence: docs/实时Agent控制舱需求与实施路线-v0.1-2026-07-13.md；A/B task cards；runtime/selector/renderer fixtures；lint/build/orchestration gates；1280/1440/1920 browser DOM 与 1920 首屏 CTA 边界。
```
