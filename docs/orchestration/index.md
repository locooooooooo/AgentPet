...
- `[长工]#realtime-truth-ui@v0.1`: partial-accepted after selector plus App/Home/Cockpit truth wiring and 1280/1440 browser fallback replay; configured/online/simulated/source/session semantics are separated, while renderer SSR fresh/stale/lost fixture remains pending.
- `[长工]#realtime-requirements-control@v0.1`: active docs/control owner; records the approximately 55% bounded baseline, A/B fences, quantitative acceptance and remaining renderer fixture/trusted-authorizer/real-E2E gaps. P0-C Codex controlled dry-run remains standby pending full A/B acceptance and a second explicit execution authorization.
...
- Real-time cockpit A/B implementation is active but unaccepted. Configured seeds, CLI discovery, static orchestration roles, and simulated ticks must not be reported as online Agent sessions; P0-C dry-run remains standby.
...
next action:
- PM must independently verify the received P0-A/P0-B callbacks, file fences, M5 hunk preservation, fixtures, truth UI and joint runtime behavior; keep P0-C blocked until those checks pass and a fresh explicit Codex dry-run authorization exists.
